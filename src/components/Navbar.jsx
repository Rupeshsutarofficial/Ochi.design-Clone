...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] text-zinc-800 w-full px-6 py-6 flex justify-between items-center backdrop-filter backdrop-blur-[5px] ${
        visible ? "" : "-translate-y-full"
      }`}
      style={{ fontFamily: "NeueMontreal-Regular", transition: "transform 0.3s ease-in-out" }}
    >
      <div className="logo scale-[1.3] ml-2">
      ...
      <div className="links flex gap-7">
        {["Services", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
          <a key={index} className={`text-[1.3vw] cursor-pointer ${index === 4 && "ml-[15vw]"}`}>
            {item}
          </a>
        ))}
      </div>
